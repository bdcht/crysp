...
def test_blockiterator_001():
    ...
    with pytest.raises(PaddingError) as exc:
        next(g.iterblocks('AAA',padding=False))
    assert exc.value.value == 'input not a multiple of block size'

def test_blockiterator_002():
    g = blockiterator(32)
    B = list(g.iterblocks('A'*4,padding=False))
    assert len(B)==1
    assert g.bitcnt==32
    assert g.padcnt==0
    B = list(g.iterblocks('A'*8,padding=False))
    assert len(B)==2
    assert g.bitcnt==96
    assert g.padcnt==0
    B = list(g.new.iterblocks('A'*8,padding=False))
    assert len(B)==2
    assert g.bitcnt==64
    assert g.padcnt==0
    B = list(g.iterblocks('A'*8,bitlen=32,padding=False))
    assert len(B)==1
    assert g.bitcnt==96
    assert g.padcnt==0
...
def test_blockiterator_003():
    g = blockiterator(32)
    B = list(g.iterblocks('A'*9,bitlen=64,padding=False))
    assert len(B)==2
    assert g.bitcnt==64
    assert g.padcnt==0
    with pytest.raises(PaddingError) as exc:
        B = list(g.iterblocks('A'*9,padding=False))
    assert 'input not a multiple of block size' == exc.value.value
    B = list(g.iterblocks('A'*12,bitlen=64,padding=False))
    assert len(B)==2
    assert g.bitcnt == 128
    with pytest.raises(PaddingError) as exc:
        B = list(g.iterblocks('A'*7,bitlen=64,padding=False))
    assert exc.value.value == 'input bitlen mismatch'

def test_nopadding_001():
    pad = nopadding(16)
    c = 0
    for b in pad.iterblocks('A'*20):
...
def test_Nullpadding_001():
    ...
    assert b[-4:]=='\0'*4
    assert pad.remove(b)=='AAAA'

def test_Nullpadding_002():
    pad = Nullpadding(64)
    for b in pad.iterblocks('A'*25,bitlen=192):
        assert len(b)==8
    assert pad.padflag
    assert pad.padcnt == 0
...
def test_bitpadding_003():
    pad = bitpadding(64)
    for b in pad.iterblocks('A'*8,bitlen=63):
        assert len(b)==8
    assert pad.padflag
    assert pad.bitcnt==63
    assert pad.padcnt == 1
    assert ord(pad.remove(b)[-1])&0xfe == ord('A')&0xfe

def test_pkcs7_001():
    pad = pkcs7(64)
    B = list(pad.iterblocks('A'*17))
    assert len(B)==3
    assert pad.padflag
    assert pad.bitcnt==17*8
    assert pad.padcnt==56
    assert B[-1][-1]=='\x07'
    assert pad.remove(B[-1])=='A'
